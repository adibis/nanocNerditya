module PostHelper
  ...
  def get_post_start(post)
    ...
    if content =~ /\s<!-- more -->\s/
      ...
      "<a href='#{post.path}'><button type='button' class='btn btn-default'>Continue Reading &rarr;</button></a>"
    end
    return content
  end
end
...
module Nanoc3::Helpers::Blogging
	...
	def month_link_with_count(month, count)
		...
		%{<a href="/archives/#{permalink}.html">#{month} <span class="badge pull-right">#{count}</span></a>}
	end
